...
local i=(function()local j=false;local function k(l)
      
      
      
      local function i(...)return...end;
      local m,n,o,p,q=string.char,string.byte,math.floor,table.insert,table.concat;local function r(s)local t,u,v,w,z,A=1.0,0.0,0.0,7^18,5^22,s:gmatch"%S"local B,C,D,E,F,G={},{},0x7F,0x7F;local function H(I,J)while t<=94^7 do t,u,v=t*94.0,u*94.0+n(A())-33.0,(v-o((v+t-1)/94^7)*94^7)*94.0 end;if I then local K=t%I;local L=(t-K)/I;local M=L+1.0;local N=K*M;local O,P,Q,R;if u<N then t=M;P=u-u%M;O=P/M else t=L;P=u-(u-N)%L;O=K+(P-N)/L end;local S,T=1.0,O;if J then T,Q,R=4,0,I;repeat local U=J[T]if O<U then R=U else Q,T=U,T+1 end;T=J[T+1]until T<0;O,S=Q,R-Q;P=Q<K and Q*M or N+(Q-K)*L;t=(R<K and R*M or N+(R-K)*L)-P end;v,u=v+P,u-P;w,z=w%93471801.0*(z%93471811.0)+I,w%93471821.0*(z%93471831.0)-O*773.0-S*7789.0;return T end;assert((w-z)%t==u)end;local function V(W)local J=C[W]local X=J[3]if X==0.0 then local T=H(J[1],J)local Y=J[2]X=T%Y;T=(T-X)/Y+2.0;X=2.0^X;X=X+H(X)if T~=0.0 then return T*X end end;J[3]=X-1.0 end;for W=1,2 do local Z=H(2^15)local Y=H(17)local J,_={Z,Y,0.0},3*Y;local function a0(Q,R,a1)local U,a2=Q+1;U=H(R-U)+U;J[a1],a1=U,a1+3;for a3=a1-2,a1-1 do if H(2)==1 then a2,a1=a1,a0(Q,U,a1)else a2=H(_)-_ end;J[a3],Q,U=a2,U,R end;return a1 end;a0(0,Z,4)C[W]=J end;while true do local a4=V(1)if not a4 then a4=F elseif a4==F then H()return B end;D,F,a4=D+a4,a4,V(2)or G;E,G=E+a4,a4;B[D]=E end end;if j then print"-------------------------------------------------"print"Testing data integrity of all compressed mappings"for a5,s in pairs(l)do print(a5,pcall(r,s)and"OK"or"FAILED")end;print"-------------------------------------------------"end;if(os.getenv"os"or""):match"^Windows"then 
      
      local function a6()
            print"try to retrieve ansi codepage"
            local a7=assert(io.popen[[reg query HKLM\SYSTEM\CurrentControlSet\Control\Nls\CodePage /v ACP]])local a8=a7:read"*a"
      print("reg query result:"..a8)local a5=a8:match"%sACP%s+REG_SZ%s+(.-)%s*$"a7:close()if not a5 then print"accessing Windows ANSI code page from Windows registry failed. Try again..."
            a7=assert(io.popen[[C:\Windows\System32\reg query HKLM\SYSTEM\CurrentControlSet\Control\Nls\CodePage /v ACP]])
            a8=a7:read"*a"print("reg query result:"..a8)a5=a8:match"%sACP%s+REG_SZ%s+(.-)%s*$"a7:close()end;
            return assert(a5,"Failed to determine Windows ANSI codepage from Windows registry")end;
      
      local a5=a6()local s=l[a5]if s then local a9=r(s)
            local function aa(ab,ac)ac=ac or 1;local ad,ae=n(ab,ac),1;if ad>=0xC0 and ad<0xFE then local af=64;ad=ad-128;repeat local ag=n(ab,ac+ae)or 0;if ag>=0x80 and ag<0xC0 then ad,ae=(ad-af-2)*64+ag,ae+1 else ad,ae=n(ab,ac),1 end;af=af*32 until ad<af end;return ad,ae end;
      
      
      function i(ab)
            local ac,ah=1,{}
            while ac<=#ab do local ad,ae=aa(ab,ac)ac=ac+ae;ad=ad<128 and ad or a9[ad]or n"?"
                  if ad>255 then p(ah,m(o(ad/256)))end;p(ah,m(ad%256))end;return q(ah)end;local ai=os.rename;function os.rename(aj,ak)return ai(i(aj),i(ak))end;local al=os.remove;function os.remove(am)return al(i(am))end;

local an=os.execute;

function os.execute(ao)if ao then ao=i(ao)end;return an(ao)end;

local ap=io.open;function io.open(am,...)return ap(i(am),...)end;
local aq=io.popen;function io.popen(ar,...)return aq(i(ar),...)end;
local as=io.lines;function io.lines(am,...)if am then am=i(am)return as(am,...)else return as()end end;
local at=dofile;function dofile(am)if am then am=i(am)end;return at(am)end;
local au=loadfile;function loadfile(am,...)if am then am=i(am)end;return au(am,...)end;local av=require;
function require(aw)aw=i(aw)return av(aw)end;local ax=io.input;
function io.input(ay)if type(ay)=="string"then ay=i(ay)end;return ax(ay)end;
local az=io.output;function io.output(ay)if type(ay)=="string"then ay=i(ay)end;return az(ay)end else end end;return i end;

return k{["874"]=[[!%l+$"""WN^9=&$pqF'oheO#;0l#"hs)mI[=e!ufwkDB#OwLnJ|IRIUz8Q(MMM]],["1250"]=[[!<2#?v"1(ro;xh/tL_3hC^i;e~PjO"p<I\aTT};]Rb~M7/]&jRjfwuE%AJ)@XfBQy&\jy[V5:]!RtH]m>Yd8m?6LpsUA\V=x'VcMO<Wz+EOO
      0m7U`u|$Y5x?Vk*6+qJ@/0Lie77_b}OEuwv$Qj/w`+J>M*<g2qxD3qEyC&*{VGI'UddQ`GQ)L=lj<{S;Jm),f3yzcQOuxacHSZ{X'XIWzDz!?E
      =U0f]],["1251"]=[[!-[;_8kMai7j]xB$^n)#7ngrX}_b%{<Cdot;P?2J&00&^wX|;]@N*fjq#ioX'v.&gG@ur~3yi8t1;xn40{G#NX?7+hGC{$D"4#oJ//~kflzs
      "_\z9qP#}1o|@{t`2NrM%t{MW?X9d6o:MqHl6+z]],["1252"]=[[!)W$<c~\OdA5TJ%/J/{:yoE]K[d,c<Mv+gp_[_UuB52c;H&{leFk%Kd8%cHnvLrB[>|:)t.}QH*)]AD|LqjsB+JCdKmbRIjO,]],["1253"]=[[!./yDCq;#WAuC\C1R{=[n'FpSuc!"R\EZ|4&J?A3-z?*TI?ufbhFq1J!x@Sjff\!G{o^dDXl|8NLZ!$d'8$f^=hh_DPm!<>>bCgV(>erUWhX
      ?R+-JP@4ju:Yw#*C]],["1254"]=[[!-(R[SPKY>cgcK5cCs4vk%MuL`yFx^Bl#/!l#M@#yoe|Jx+pxZuvh%r>O</n_gb>hDjmG]j#lA{]2"R-Z@(6Wy:Q~%;327b&fRSkF#BM/d+%
      iWmSx4E*\F_z=s>QeJBqC^]],["1255"]=[[!.b\.H?S\21+7efm'`w&MW_Jg,mRbB;{X@T\3::DC#7<m_cAE!:%C%c7/,./u[8w*h-iwpz03QY,ay%]MI*D]W&]UG^3(=20a7$zG[Ng7MLt
      sXIne(V37A?OO%|Hn13wMh-?^jNzhW`,-]],["1256"]=[[!3n8GE$.to/ka%Nx`uOpcib>|9KU-N72!1J4c2NAUE3a,HlOE=M`@rsa||Nh_!og]:dILz9KNlF~vigNH*a0KxwjjfR*]?tO87(a3-RQex^V
      Ww&SY{:AqE|s%}@U8%rKcr0,NCjR:N&L'YyGu<us'sN*1pl=gAXOwSJ[v?f;imBhDu_)d$F8T?%S[]],["1257"]=[[!:<_.XQ[;n35s%I?g9)b/7DiGwIR)zy&=6?/3)6iO%rSnC_6yjl'8#zeN0vcW_yX/2*J93+EJVrW,^Rhe,h7wWl"}neF2~F[PyD;BcrG*5=J
      fh<x!FJ?qSw9Xp!;WB3T<J^x?#Ie`xufezR'\I(eED]3d&)VJL$/+$Zf;W^I>L[3D5F<_IcGpn=oX"JR1%arS|FX|dia4]BeF>d5p`EV+:;*I<
      x^Voq{"f]],["1258"]=[[!3n8C{%C0}&p3gE0~|&RVm9Wr&^ln1}'$gV{bml1oByN*bb:Bm^E;~B3-WjF6Qubq^`Y*6\0^w!DKpK<\7lHVELmSXN{2~B"0C"<1CYN2{$a
      5M?>|7%~qm{pXphwm3$}iyXjBYwtGqxp(f[!g^Ee9H.}1~0H-k-dzNDh1L]],["932"]=[=[3+=&7~sLTy"n^PnVG*Gx&j>4Lxml%}fe|7_}nbsu1ImFj_eqZ%/jkS#qT2PD9?]Kq'fK>~C.Ymj<?gN+{6t]jtMgZWZpx>#VyzuhHQ=Nhul
      ...
      rrlfRll2LI=b]Rkrm}-&"n%i'!K+]WaMYm*=Oei<bNeb^(VvkhH~{BcMYa*uWILR'h?5nFKa/rR6jFy"eZSZl{H%p]SErhus\u9.0Y+4Upe]=],["949"]=[=[.?P*"%JdJY.\dApak>>jkl@@vlOMP(2HYtDqZ=CE{6w89$-QMAmM=yjyuext/9^>o"(n}rRN0XD|eH}+M7fc^f"gV;sYA&4"-]gYHf.oIGZ
      ...
      *-Gfn^(mdx(]+]4/>fE2!/D$naK0]uPhG!]jM|1k*m3}PN]|a4CheAMn~7{2/^D/I;f`!}A+-]f~(!xA`C=*dZ0k)V3m+Tk5nMm[.2NU#mE2JZ
      ...
      VkQsXY^)UsoA?K_Wj{DVFXQ&80/w}U$kO?lH3@Khu3(n-O+R7hDQKlE?]=],["936"]=[=[!cq%Lf7]BuTvd\U$`xY;Ze%;A&P\kuvz~3R?dWHc6_}0*g^iH5?2RM4s9j-8~\oon,AgsbSH_mPJl-8Y@4D@,'P#{#~/"O[/w[;cmtOz:%p
      ...
      *m,<}.@9+shUu9<:!x3S98|;>"<9x{[>`@-1@=*y0eg)nz%oX[Rwe;*l9U1<<%Q*_BvILJ:69Ibw58m4EOj&%od#pVqei#t15&^T-U%8)1%P`l
      ...
      rY4st-[QX!\zm3i9lbCx{m/)Z:1~+;nk*K8&';pKk,3]uM=JeV<lx8!nZ8S6\8|yza8)sj!RBj[)5!tK]=],["950"]=[=[83_O$Pic'CxRvF+;*M|!,mpBB[ewt(*+kIL]oI'*:[WMNP9+4i'J{0,}XWh[:~fla'zX|}}c00F{qc4j$!4C}NF)OkYGR342Us#NGA~1}}t
      ...
      *Y)LU|PSf-Iq%e}r<p:q_n?&im{$sP~:P9-N-e`g86#nyu9@,@s"[-Ls^}fg?N"*B6us~E0T3ju+l+rsTW@jCP1Ww](#ROX&{gz4+!ZAhcAv`%
      ...
      ZXcFgkq"'Fy1Y&1(9JUd}Ocsef`q@Mqhp9qF,L&bZ%{fJM?;=gO;]2C,fTC!8N)06E3>|]=]}end)()
      
      
      -------地址分隔符/\
      local a_=package.config:sub(1,1)

      ---------------------------
      
      local function b0()
            local b1=FuPLATFORM_WINDOWS and'Windows'or FuPLATFORM_MAC and'Mac'or FuPLATFORM_LINUX and'Linux'
            return b1 end;

            local function b2()return b0()=='Mac'end;local function b3()return b0()=='Windows'end;
            local function b4()return b0()=='Linux'end;
      local function b5(b6)
            local ay,b7=io.open(b6,"r")if ay==nil then print("could not open file "..b6)print(b7)error(b7)end;
            local b8=ay:read("*a")ay:close()return b8 end;

local function b9(ba)
      local h=io.open(ba,"r")
      if h~=nil then io.close(h)
            return true 

else return false end end;

local function bb(ay)local bc,bd,ad=os.rename(ay,ay)if not bc then if ad==13 then return true end end;return bc,bd end;local function be(bf)if b3()then os.execute('rmdir "'..bf..'"')else os.remove(bf)end end;

local function bg(bh,bi)local bj=fusion:MapPath("Temp:/")local e=1;local bk=bj..bh..e;while bb(bk)do e=e+1;bk=bj..bh..e end;if bi then os.execute('mkdir "'..bk..'"')end;return bk end;local function bl(bm,bn,bo)local function bp(bq)if bo==nil then return false end;for br,bs in ipairs(bo)do if bs==bq then return true end end;return false end;local bj=fusion:MapPath("Temp:/")math.randomseed(os.time())local e=math.random(100000,999999)local bk=bj..bm..e..bn;while bb(bk)or bp(bk)do e=e+1;bk=bj..bm..e..bn end;return bk end;local function bt(bu)if#bu<2 then return bu end;local bv=string.sub(bu,1,1)local bw=string.sub(bu,-1,-1)if bv=="'"and bw=="'"or bv=='"'and bw=='"'then return string.sub(bu,2,-2)end;return bu end;

local function bx(bu)return bu:gsub("^%s*(.-)%s*$","%1")end;
function StringRemovePrefix(bu,by)if bu:sub(0,#by)==by then bu=bu:sub(#by+1)end;return bu end;local function bz(bA,bB,bC)if not bC then bC={}end;local t,bD=500,250;if bC.height then bD=bC.height end;local bE=false;if fusion==nil then print("[Fusion Error] Please open up the Fusion GUI before running this tool.\n")return false end;if comp~=nil then comp:Print("\n["..tostring(bA).."] "..tostring(bB).."\n")end;

if not ui then ui=app.UIManager;disp=bmd.UIDispatcher(ui)end;local bF=disp:AddWindow({ID="confirmWin",Target="confirmWin",WindowTitle=tostring(bA),ui:VGroup{ui:Label{ID="Title",Weight=0.25,Text=tostring(bA),Alignment={AlignHCenter=true,AlignVCenter=true},Font=ui:Font{PixelSize=18}},ui:Label{ID="Message",Text=tostring(bB),Alignment={AlignHCenter=true,AlignVCenter=true},WordWrap=true},ui:HGroup{Weight=0,ui:HGap(0,2.0),ui:Button{ID="CancelButton",Text="取消"},ui:Button{ID="OkButton",Text="好的"}}}})bF:Resize({t,bD})itm=bF:GetItems()function bF.On.confirmWin.Close(bG)disp:ExitLoop()end;function bF.On.CancelButton.Clicked(bG)disp:ExitLoop()end;function bF.On.OkButton.Clicked(bG)bE=true;disp:ExitLoop()end;app:AddConfig("confirmWin",{Target{ID="confirmWin"},Hotkeys{Target="confirmWin",Defaults=true,CONTROL_W="Execute{ cmd = [[ app.UIManager:QueueEvent(obj, 'Close', {}) ]] }",CONTROL_F4="Execute{ cmd = [[ app.UIManager:QueueEvent(obj, 'Close', {}) ]] }"}})bF:Show()disp:RunLoop()bF:Hide()return bE end;

local function bH(ao)
      local bI=ao.." 2>&1"
      if b3()then bI='"'..bI..'"'
      end;
      local bJ,bd=io.popen(bI)
      if bJ==nil then error((''.."无法执行")..tostring(ao))
      end;
      local bE=bJ:read("*a")
      bJ:close()return bE 
end;
      local function bK(ay,bL)
            if not b2()and not b4()
            then print("file permissions can only be checked on Mac and Linux")return 
            end;
            print("check file permissions for file "..ay)
            if not bmd.fileexists(ay)then print("file does not exist!")error("missing file '"..ay.."'")end;
            local bM=bx(bH('ls -l "'..ay..'"'))print("ls -l:"..bM)local e,bN=string.find(bM,"...x..x..x")if e==1 then print("file permissions are ok (executable for everybody)")else print("file is not executable for everybody!")print("attempt to fix permissions")local bO='chmod a+x "'..ay..'"'print(bO)local bP=bx(bH(bO))print(bP)local bQ=bx(bH('ls -l "'..ay..'"'))print("ls -l:"..bQ)e,bN=string.find(bQ,"...x..x..x")if e==1 then print("fixed permissions successfully!")else print("could not fix permissions - please make sure that this file is executable for everybody:"..ay)print("to fix the problem, please run this in Mac OS Terminal (might need to be done with admin rights):")print(bO)if not bL then bz("file has wrong permissions","The following file cannot be executed due to wrong file permissions:\n"..ay.."\n\n".."Fix: Paste the following command in the program 'Terminal' and hit return on your keyboard to execute it:\n"..bO.."\n".."\n".."If the command does not show any output, it worked. If you see an error message, you might need to execute it as administrator.\n",{height=400})end;error("file has no execute permissions")end end end;
            
            
            local function bR(b6)local bS="^[-~_/.%w%%+,:@^]*$"local function bT(bB,bU)if bB==""then bB='""'elseif not bB:match(bS)then if bU then bB='"'..bB:gsub('["\\]','\\%0')..'"'else local bV={}for bu in(bB.."'"):gmatch"(.-)'"do bV[#bV+1]=bu:match(bS)or"'"..bu.."'"end;bB=table.concat(bV,"\\'")end end;return bB end;local function bW(b6,bX)local function bY(bZ)local b_=""while bZ>0 do b_=b_.."\\"bZ=bZ-1 end;return b_ end;if bX==nil then bX=false end;if not bX and b6~=""and not string.match(b6," ")and not string.match(b6,"\t")and not string.match(b6,"\n")and not string.match(b6,"\v ")and not string.match(b6,'"')then return b6 end;local bE='"'local e=0;while e<#b6+1 do local c0=0;while e<#b6 and string.sub(b6,e+1,e+1)=="\\"do c0=c0+1;e=e+1 end;if e==#b6 then bE=bE..bY(2*c0)elseif string.sub(b6,e+1,e+1)=='"'then bE=bE..bY(2*c0+1)..'"'else bE=bE..bY(c0)..string.sub(b6,e+1,e+1)end;e=e+1 end;bE=bE..'"'return bE end;if b2()then return bT(b6,false)else return bW(b6)end end;
      ...
      local function cE(cF)
            local ao=''
            local b1=b0()
            if b1=='Windows'then ao='explorer "'..cF..'"'elseif b1=='Mac'then ao='open "'..cF..'" &'elseif b1=='Linux'then ao='xdg-open "'..cF..'" &'end;print('[启动命令] ',ao)os.execute(ao)end;
            
            local function cG(bA,bB)local t,bD=500,250;if fusion==nil then print("[Fusion Error] Please open up the Fusion GUI before running this tool.\n")return else if comp~=nil then comp:Print("\n["..tostring(bA).."] "..tostring(bB).."\n")end;if ERROR_WINDOW_CONSOLE_MODE~=true then if not ui then ui=app.UIManager;disp=bmd.UIDispatcher(ui)end;

local bF=disp:AddWindow(
      {ID="errWin",Target="errWin",WindowTitle=tostring(bA),

ui:VGroup{ui:Label{ID="Title",Weight=0.25,Text=tostring(bA),Alignment={AlignHCenter=true,AlignVCenter=true},Font=ui:Font{PixelSize=18}},

ui:Label{ID="Message",Text=tostring(bB),Alignment={AlignHCenter=true,AlignVCenter=true},WordWrap=true},

ui:HGroup{Weight=0,ui:HGap(0,2.0),ui:Button{ID="OkButton",Text="好的"}}}})

bF:Resize({t,bD})

itm=bF:GetItems()

function bF.On.errWin.Close(bG)
disp:ExitLoop()end;
function bF.On.OkButton.Clicked(bG)
disp:ExitLoop()end;

app:
AddConfig("errWin",{Target{ID="errWin"},
 Hotkeys{Target="errWin",Defaults=true,CONTROL_W="Execute{ cmd = [[ app.UIManager:QueueEvent(obj, 'Close', {}) ]] }",CONTROL_F4="Execute{ cmd = [[ app.UIManager:QueueEvent(obj, 'Close', {}) ]] }"}})

bF:Show()disp:RunLoop()
bF:Hide()
...
-- if not ui then ui=app.UIManager;disp=bmd.UIDispatcher(ui)end;

-- local bF=disp:AddWindow({ID=''..g('bGljZW5zZURsZw=='),Target=''..g('bGljZW5zZURsZw=='),WindowTitle=tostring(bA),ui:VGroup{Spacing=4,ui:HGroup{ui:Label{ID=''..g('TGFiZWxUZXh0MQ=='),Text=""}},ui:HGroup{ui:Label{ID=''..g('TGFiZWxUZXh0Mg=='),Text=""}},ui:HGroup{Weight=0,ui:LineEdit{ID=''..g('TGljZW5zZVRleHQ='),Text=""}},ui:VGroup{ID=''..g('RXJyb3JHcm91cA=='),Weight=3,ui:HGroup{ui:Label{ID=''..g('TGFiZWxFcnJvck5hbWU='),Text=""},ui:Button{Weight=0,ID=''..g('R2V0SGVscEJ1dHRvbg=='),Text=''..g('R2V0IEhlbHA=')}},ui:Label{ID=''..g('TGFiZWxFcnJvckRldGFpbA=='),Text="",WordWrap=true}},ui:VGap(10,0),ui:HGroup{Weight=0,ui:VGroup{ui:Button{ID=''..g('UmV0cmlldmVMaWNlbnNlQnV0dG9u'),Text=''..g('UmV0cmlldmUgTGljZW5zZQ==')},ui:Button{ID=''..g('QnV5TGljZW5zZUJ1dHRvbg=='),Text=''..g('QnV5IExpY2Vuc2U=')}},ui:HGap(0,2.0),ui:Button{ID=''..g('Q2FuY2VsQnV0dG9u'),Text=''..g('Q2FuY2Vs')},ui:Button{ID=''..g('T2tCdXR0b24='),Text=''..g('T2s=')}}}})local function cP()local cQ=ch[''..g('cmVzdWx0')]
--       local cR=ch[''..g('TFM=')]==''..g('T0s=')and ch[''..g('TFNT')]==''..g('VVA=')itm.LabelText1.Text=''..g('UGxlYXNlIGVudGVyIHRoZSBsaWNlbnNlIGNvZGU=')if cR then print(''..g('bGljZW5zZSBzZXJ2ZXIgaXMgcnVubmluZyBhbmQgY29uZmlndXJlZA=='))itm.LabelText2.Text=''..g('QSBmbG9hdGluZyBsaWNlbnNlIHNlcnZlciB3YXMgZm91bmQgb24gdGhlIG5ldHdvcmssIGVudGVyIEBSRU1PVEUgdG8gcmVxdWVzdCBhIGxpY2Vuc2U=')itm.LicenseText.Text=''..g('QFJFTU9URQ==')else print(''..g('bGljZW5zZSBzZXJ2ZXIgaXMgbm90IHJ1bm5pbmcgb3Igbm90IGNvbmZpZ3VyZWQ='))itm.LabelText2.Text=''..g('VG8gcnVuIGluIHRyaWFsIG1vZGUgdHlwZTogdHJpYWw=')local co=ch[''..g('dHJpYWwgZGF5cyBsZWZ0')]if co~=nil then itm.LabelText2.Text=itm.LabelText2.Text..(''..g(''))..co..''..g('IGRheXMgbGVmdCk=')end;

-- itm.LicenseText.Text=''..g('dHJpYWw=')end;if cQ>0 then itm.LabelErrorName.Text=""itm.LabelErrorDetail.Text=""cO=nil;itm.ErrorGroup:Hide()bF:Resize({t,bD})else local cS=cD(cQ)local cT=cS[''..g('dGl0bGU=')]local cU=cS[''..g('ZGV0YWls')]if cT==nil then cT=""end;if cU==nil then cU=""end;print(''..g('ZXJyb3IgbWVzc2FnZTo='))print(cT)print(cU)itm.LabelErrorName.Text=(''..g('PGZvbnQgY29sb3I9JyNlNjRiM2QnID4='))..cT.." ("..cQ..")".."</font>"itm.LabelErrorDetail.Text=(''..g('PGZvbnQgY29sb3I9JyNlNjRiM2QnID4='))..cU.."</font>"cO=cS[''..g('dXJs')]itm.ErrorGroup:Show()bF:Resize({t,bD})end end;bF:Resize({t,bD})itm=bF:GetItems()cP()bF:Resize({t,bD})

-- function bF.On.CancelButton.Clicked(bG)disp:ExitLoop()end;

-- function bF.On.OkButton.Clicked(bG)disp:ExitLoop()end;

----local cV=bx(itm.LicenseText.Text)local bE=ch[''..g('cmVzdWx0')]

-----if cV==''..g('dHJpYWw=')
-- then if bE~=aJ and bE~=aT then ch=cw()bE=ch[''..g('cmVzdWx0')]end;
-- print((''..g('c3RhcnQgaW4gdHJpYWwgbW9kZSAo'))..bE..")")
-- else ch=ct(cV)end;

-----bE=ch[''..g('cmVzdWx0')]if bE>=0 then if bE~=aJ then cG(''..g('VGhhbmsgeW91IQ=='),cH(ch))end;

---disp:ExitLoop()else cP()end 

----end;


-- function bF.On.RetrieveLicenseButton.Clicked(bG)cE(''..g('aHR0cHM6Ly9hZXNjcmlwdHMuY29tL2Rvd25sb2FkYWJsZS9jdXN0b21lci9wcm9kdWN0cy8='))end;function bF.On.GetHelpButton.Clicked(bG)cE(cO)end;function bF.On.BuyLicenseButton.Clicked(bG)cE(aH)end;function bF.On.licenseDlg.Close(bG)disp:ExitLoop()end;bF:Show()disp:RunLoop()bF:Hide()return ch end;local function cW()local ch;local itm;local t,bD=500,250;local bA=aG..(''..g('IA=='))..aI;if fusion==nil then print("[Fusion Error] Please open up the Fusion GUI before running this tool.\n")return ch end;if not ui then ui=app.UIManager;disp=bmd.UIDispatcher(ui)end;

-- local bF=disp:AddWindow({ID=''..g('bGljZW5zZUhlbHBEbGc='),Target=''..g('bGljZW5zZUhlbHBEbGc='),WindowTitle=tostring(bA),ui:VGroup{Spacing=4,ui:HGroup{ui:TextEdit{HTML=LicenseSystemHelpInfoText,ReadOnly=true,Alignment={AlignHCenter=true,AlignTop=true}}},ui:HGroup{Weight=0,ui:Label{ID=''..g('TGljZW5zZUluZm9UZXh0'),Text=''..g('bGljZW5zZSBjaGVjayBpbiBwcm9ncmVzcy4uLjxici8+Jm5ic3A=')}},ui:HGroup{
--       ui:VGroup{Weight=0,ui:Button{ID=''..g('TGljZW5zZUJ1dHRvbg=='),Text=''..g('RGVhY3RpdmF0ZSBMaWNlbnNl')},ui:Button{ID=''..g('U3VwcG9ydEJ1dHRvbg=='),Text=''..g('R2V0IHN1cHBvcnQ=')}},
--       Weight=0,ui:HGap(0,2.0),ui:Button{ID=''..g('T2tCdXR0b24='),Text=''..g('T2s=')}
-- }}})itm=bF:GetItems()

-- local function cP()print("updateUITexts")itm.LicenseInfoText.Text=cH(ch)if ch[''..g('cmVzdWx0')]==aJ then itm.LicenseButton.Text="Buy License"else itm.LicenseButton.Text="Deactivate License"end end;bF:Resize({t,bD})function bF.On.OkButton.Clicked(bG)disp:ExitLoop()end;
-- function bF.On.SupportButton.Clicked(bG)
-- cE((''..g('aHR0cHM6Ly9hZXNjcmlwdHMuY29tL2NvbnRhY3QvP2RpcmVjdD0xJnNrdT0='))..aC)
-- disp:ExitLoop()end;function bF.On.LicenseButton.Clicked(bG)if ch[''..g('cmVzdWx0')]==aJ then cE(aH)else cw()end;disp:ExitLoop()end;function bF.On.licenseHelpDlg.Close(bG)disp:ExitLoop()end;bF:Show()ch=cx()cP()disp:RunLoop()bF:Hide()end;

local function cX()local cY=os.time()local cZ=os.time(aF)local c_=aE and cY>cZ;
      if c_ then cG(''..g('VGhpcyBCZXRhIHZlcnNpb24gaXMgZXhwaXJlZA=='),''..g('UGxlYXNlIGluc3RhbGwgYSBuZXdlciB2ZXJzaW9uIG9mIHRoZSB0b29sLg=='))
            return false end;local ch=cx()local bE=ch[''..g('cmVzdWx0')]if bE==aK then local d0=ch[''..g('YV9uZXh0')]
                  local d1=d0*-1;cG(''..g('QWN0aXZhdGlvbiBSZXF1aXJlZA=='),(''..g('UGxlYXNlIGNvbm5lY3QgdG8gdGhlIGludGVybmV0IG9yIGNvbmZpZ3VyZSB5b3UgZmlyZXdhbGwgdG8gYWxsb3cgdGhlIGxpY2Vuc2UgdG8gYmUgY2hlY2tlZCBhbmQgYWN0aXZhdGVkIG9ubGluZS4gWW91IGhhdmUg'))..d1 ..''..g('IGRheXMgdG8gYWN0aXZhdGUgaXQgdW50aWwgdGhlIGxpY2Vuc2UgYmVjb21lcyBpbnZhbGlk'))end;if bE==aJ or bE<0 then ch=cN(ch)bE=ch[''..g('cmVzdWx0')]end;
                  return bE>=0 end;
        --------获取脚本程序路径----------------------------------         
        ...
        local function getScriptPath()
            local function getSourcePath()
              ...
              if debugInfo.source:sub(1, 1) == "@" then
                return debugInfo.source:sub(2)
              else
                return nil
              end
            end
            ...
            local scriptPath = sourcePath:match(".*" .. a_)
          ...
          local function d2()
            local function createExtensionPath(directory)
              return directory
            end
          
            local function checkDirectoryExists(directory)
              local exists = bmd.direxists(directory)
              if exists then
                print("found extension files at " .. directory)
              else
                print("did not find extension files at " .. directory)
              end
              return exists
            end
            ...
            if scriptPath == nil then
                  print("无法直接获取脚本目录 ")
              local directory
              local scriptsPath = fusion:MapPath("Scripts:")
              local allDataPath = fusion:MapPath("AllData:")
          
              if scriptsPath then
                directory = createExtensionPath(scriptsPath .. "Utility" .. a_ .. "sundayImage" .. a_)
                if checkDirectoryExists(directory) then return directory end
          
                directory = createExtensionPath(scriptsPath .. "Utility" .. a_ .. "sundayImage tools" .. a_)
                if checkDirectoryExists(directory) then return directory end
          
            --     directory = createExtensionPath(scriptsPath .. "Utility" .. a_)
            --     if checkDirectoryExists(directory) then return directory end
              end
          
              if allDataPath then
                directory = createExtensionPath(allDataPath .. "Scripts" .. a_ .. "Utility" .. a_ .. "sundayImage" .. a_)
                if checkDirectoryExists(directory) then return directory end
          
                directory = createExtensionPath(allDataPath .. "Scripts" .. a_ .. "Utility" .. a_ .. "sundayImage tools" .. a_)
                if checkDirectoryExists(directory) then return directory end
          
            --     directory = createExtensionPath(allDataPath .. "Scripts" .. a_ .. "Utility" .. a_)
            --     if checkDirectoryExists(directory) then return directory end
              end
          
              error("GetFolderOfCurrentScript failed")
            else
              return createExtensionPath(scriptPath)
            end
          end
...
local function d5(ay)local bE=ay:match("^.+[/\\](.+)$")if bE~=nil then return bE end;return ay end;

-----提取文件名
local function d6(ay)
      local bE=ay:match("(.+)%..+")
      if bE~=nil then return bE end;
      return ay end;
      assert(d6("foo.bar")=="foo","remove file extension failed 1")assert(d6("foobar")=="foobar","remove file extension failed 2")assert(d6("baz/foo.bar")=="baz/foo","remove file extension failed 3")
      
      
      local function d7(ay)
            local bE=ay:match('(.*'..a_..')')if bE~=nil then return bE end;return""end;
            local function d8(by)by=StringRemovePrefix(by,"/Volumes/Macintosh HD")return by end;local function bg(bh,bi)local bj=fusion:MapPath("Temp:/")local e=1;local bk=bj..bh..e;while bb(bk)do e=e+1;bk=bj..bh..e end;if bi then os.execute('mkdir "'..bk..'"')end;return bk end;
      
      local function bl(bm,bn,bo)
            local function bp(bq)
                  if bo==nil then return false end;
                  for br,bs in ipairs(bo)do 
                        if bs==bq then return true 
                  end 
            end;
            return false 
      end;
            local bj=fusion:MapPath("Temp:/")math.randomseed(os.time())local e=math.random(100000,999999)
            local bk=bj..bm..e..bn;
            while bb(bk)or bp(bk)do e=e+1;bk=bj..bm..e..bn end;
            return bk 
      end;

            local function bg(bh,bi)local bj=fusion:MapPath("Temp:/")local e=1;local bk=bj..bh..e;while bb(bk)do e=e+1;bk=bj..bh..e end;if bi then os.execute('mkdir "'..bk..'"')end;return bk end;
            
            local function bl(bm,bn,bo)
                  local function bp(bq)
                        if bo==nil then return false end;
                        for br,bs in ipairs(bo)do 
                              if bs==bq then return true end end;
                              return false end;
            local bj=fusion:MapPath("Temp:/")math.randomseed(os.time())local e=math.random(100000,999999)
            local bk=bj..bm..e..bn;
            while bb(bk)or bp(bk)do e=e+1;bk=bj..bm..e..bn 
            end;
            return bk end;
----cc是C:\Users\Sunda\AppData\Local\Temp\command125197.bat
      local function d9(ao,da)
            local cc=bl(''..'command',''..'.bat')
    print((''.."生成临时bat:")..cc)
            local cd, bd = io.open(cc, "w")
             if cd == nil then
            print("无法打开文件：" .. cc)
               print(bd)
                error(bd)
                  end
                cd:write(ao)  -- 将内容写入批处理文件
                  cd:close()

                  if not b9(cc)then print("bat file does not exist altough I just created it!")end;
                  local bE
                  -- 启动 BeatEdit.bat 文件
                  if da then
                    bE = os.execute(bR(cc))  
                  else
                    local bJ, db = io.popen(cc)
                    if bJ == nil then
                      print(db)
                      error('无法执行批处理文件')
                    end
                  ...
                  os.remove(cc)print("批处理文件执行结果:"..tostring(bE))return bE end;

local function dc()
      local dd=d2().."BeatEditForResolve_Files"..a_.."ffmpeg"..a_;if b2()then dd=dd.."Mac"..a_.."ffmpeg"elseif b3()
      then dd=dd.."Win"..a_.."ffmpeg.exe"elseif b4()then dd=dd.."Linux"..a_.."ffmpeg"else error("This operating system is not supported ")end;
      return dd 
end;
      local function de(df)
            local dg='"'..dc()..'" '..df;return dg 
      end;
     
     --------匹配生成文件与源文件位置
      local function dh(df,bC)
            if not bC then bC={}
            end;
            if bC.expectFile then bC.successCheck=function()
                  return b9(bC.expectFile)
            end 
            end;
      local dg=de(df)
      local function di()
            print('[Launch FFMPEG Command] '..dg)

local bE;
local dj=b3()
       if dj and not bC.avoidBatFiles then print("execute ffmpeg via bat file")
      bE=d9(dg)
         else print("execute ffmpeg via io.popen")bE=bH(dg)end;print("ffmpeg command execution done with result")print("---------")print(bE)print("---------")end;di()if bC.successCheck and not bC.successCheck()then print("ffmpeg success check failed")if b2()or b4()then bK(dc(),false)di()else if not bC.avoidBatFiles then print("try second method")bC.avoidBatFiles=true;dh(df,bC)end end end end;
------------dl带路径音乐文件
      local function dk(dl)
            local dm = {}
            local dn = d2() .. "BeatEditForResolve_Files" .. a_ .. "ibt" .. a_
            local dj = b3()
            local c2 = b2()
            local c4 = b4()
...
--------dp是ibtResults文件夹
           local dp=bg("ibtResults",true)
           print("save IBT results in folder "..dp)
           local dq=dp..a_.."song.wav"
           local dr=dp..a_.."song.txt"
           print("before ffmpeg args")
----------------ds，ffmpeg命令内容
           local ds='-i '..bR(dl)..' -af "aformat=sample_rates=44100|48000" -ac 2 '..bR(dq)
           print("after ffmpeg args")
           local dt;
           --------dj是Windows系统
           if dj then 
            dt='"'..dn..'"'..' -off -o beats -m "'..dq..'" "'..dp..a_..a_..'"'
           else dt=bR(dn)..' -off -o beats -m '..bR(dq)..' '..bR(dp..a_)end;
-------dn是IBT的位置
            print('[IBT Command] '..dt)
            local bE;
-----bH是读取文件
          local function du(dt)
                return bH(dt)end;

      if c2 or c4 then dh(ds,{expectFile=dq})
            if not b9(dq)then 
            print("ffmpeg seems to have failed, file does not exist:")
            print(dq)cG("请选择音频文件","无法转为wav文件:\n"..dl)error("ffmpeg failed (result file not found)")
            end;
            bE=du(dt)
               if bE:match("All Done!")==nil then print("IBT seems to have failed")
                   bK(dn,false)bE=du(dt)
                    if bE:match("All Done!")==nil then print("IBT seems to have failed (second attempt)")
                    end 
                end 
       else 
            local dv=[[
@echo If the beat detection failed, please find a way to contact me > %tmp%\autobeat_log.txt 2>&1
@echo .>> %tmp%\autobeat_log.txt 2>&1
@echo execute BeatEdit bat file for beat detection >> %tmp%\autobeat_log.txt 2>&1
...
@if not exist "]]..dp..[[\" (
	echo folder does not exist "]]..dp..[[\" >> %tmp%\autobeat_log.txt 2>&1
	echo create folder >> %tmp%\autobeat_log.txt 2>&1
	mkdir "]]..dp..[[" >> %tmp%\autobeat_log.txt 2>&1
)
@if not exist "]]..dp..[[\" (
	echo could not create folder, maybe a permission issue? >> %tmp%\autobeat_log.txt 2>&1
        exit /b
)
@if exist "]]..dq..[[" (
	echo song.wav exists already, will delete it >> %tmp%\autobeat_log.txt 2>&1
	del "]]..dq..[["
)
@echo.
@echo process audio file
(]]..de(ds)..[[) >> %tmp%\autobeat_log.txt 2>&1

@if exist "]]..dq..[[" (
	@echo.	
	@echo 开始检测拍子 - 稍等...
	 (]]..dt..[[) >> %tmp%\autobeat_log.txt 2>&1

	@if exist "]]..dr..[[" (
		echo 完成！
		del "%tmp%\autobeat_log.txt"
) else (
	echo.
	echo --------------------------------------------------------------------------------
	echo %ESC%[101;93m Error: IBTfailed %ESC%[0m
	echo --------------------------------------------------------------------------------
	echo problem: 
	echo file does not exist: ]]..dr..[[
	echo. 
	echo %ESC%[101;93m what to do? %ESC%[0m
	echo %ESC%[93mplease find a way to contact me and include the text of the log file: 
	echo %tmp%\autobeat_log.txt
	echo Please also mention if you noticed anything else, like warnings from antivirus	
	echo software, for example, which might have blocked the process. The more details 
	echo we know, the quicker and better we will be able to help.
	echo ^(press any key to open the log file and a support ticket^)%ESC%[0m
	echo --------------------------------------------------------------------------------
	echo.
	pause
	start notepad "%tmp%\autobeat_log.txt"
	start "" "https://space.bilibili.com/24892290?spm_id_from=333.1007.0.0"
)
) else (
	echo.
	echo --------------------------------------------------------------------------------
	echo %ESC%[101;93m Error: ffmpeg failed %ESC%[0m
	echo --------------------------------------------------------------------------------
	echo problem: 
	echo file does not exist: ]]..dq..[[
	echo. 
	echo %ESC%[101;93m what to do? %ESC%[0m
	echo %ESC%[93mplease find a way to contact me and include the text of the log file: 
	echo %tmp%\autobeat_log.txt
...
exit /B 0
            ...
            bE=d9(dv,true)
            print("bat file execution result:")print(bE)
            if not b9(dr)then print("result file not found "..dr)print("try second method...")

-- 使用ffmpeg进行文件转换
dh(ds, { avoidBatFiles = true })

-- 检查转换后的文件是否存在
if not b9(dq) then
    print("ffmpeg seems to have failed, file does not exist:")
    print(dq)
    cG("请选择有音频的文件", "Could not convert file to a wav file:\n" .. dl)
    error("ffmpeg failed (result file not found after trying second method)")
end

-- 执行IBT
bE = du(dt)
if bE:match("All Done!") == nil then
    print("IBT seems to have failed (second method)")
end 
end end

-- 检查IBT生成的结果文件是否存在
if not b9(dr) then
    print("IBT seems to have failed, file does not exist:")
    print(dr)
    cG("拍子检测失败", "请联系我获取支持。\n文件未创建:" .. dr)
    error("IBTfailed (result file not found)")
end

-- 读取IBT结果文件并存入dm表中
for ci in io.lines(dr) do
    local dw = tonumber(string.match(ci, "%S+"))
    table.insert(dm, dw)
end
...
local function dy(dz)
      dz=d8(dz)
      print("search for file "..dz)local dA=resolve:GetProjectManager()
      local dB=dA:GetCurrentProject()
      local bE={}
      local function dC(dD)
            for dE,dF in ipairs(dD:GetClipList())do local dG=dF:GetClipProperty("File Path")if type(dG)=="table"then dG=dG["File Path"]end;print("check file "..dG)
            if dz==d8(dG)then print("found file!")
                  table.insert(bE,dF)end end;for dE,dH in ipairs(dD:GetSubFolderList())do dC(dH)end end;
      dC(dB:GetMediaPool():GetRootFolder())return bE end;

      -------------dL是帧速率 dJ是轨道音频
      local function dI(dJ, dm, dK, dL, dM, dN, dO, markerType)
            print("create Beat Markers("..dK..","..tostring(dL)..","..tostring(dM)..","..tostring(dN)..")")
          
            local dP = {"Blue","Cyan","Green","Yellow","Red","Pink","Purple","Fuchsia"}
            local counter = 0
          
            if markerType == "timelineMarker1" or markerType == "timelineMarker2" then
              for e, dw in ipairs(dm) do
                local dQ = dP[e % #dP + 1]
                local dR
          -----------------dR音频位置 dN片段长度
                if markerType == "timelineMarker1" then
                  dR = math.ceil(dw * dL) + dK
                else
                  dR = math.floor(dw * dL + 0.5) + dK
                end
          
            --     if dN ~= nil 
            --     and dR > dN 
            --     then return end
                if dM == nil or dM <= dR then
                  if e % 13 == 0 then
                    dO.setMessage("处理拍子 "..e.."/"..#dm, true)
                  end
          
                  dJ:AddMarker(dR, dQ, "Beat "..e, "created by SundayImage", 1.0)
                end
              end
            else
              for e, dw in ipairs(dm) do
                local dQ = dP[e % #dP + 1]
                local dR
                print("sunday" .. markerType)
          
                if counter % 2 == 0 and (markerType == "clipMarker1" or markerType == "clipMarker2")then
                  if markerType == "clipMarker1" then
                    dR = math.ceil(dw * dL) + dK
                  else
                    dR = math.floor(dw * dL + 0.5) + dK
                  end
                  -- if dN ~= nil and dR > dN then return end
                  if dM == nil or dM <= dR then
                    if e % 13 == 0 then
                      dO.setMessage("处理拍子 "..e.."/"..#dm, true)
                    end
               ...
               elseif counter % 2 == 1 and (markerType == "clipMarker3" or markerType == "clipMarker4")then
                  if markerType == "clipMarker3" then
                    dR = math.ceil(dw * dL) + dK
                  else
                    dR = math.floor(dw * dL + 0.5) + dK
                  end
            ...
            dO.clearTemporaryMessage()
      end
          

------音频标签 dT是dJ，轨道音频
local function dS(dT,dm,dO,slower)
local dU=dT:GetClipProperty("FPS")
if type(dU)=="table"then print("fps is a table")
      dump(dU)dU=dU["FPS"]end;

dI(dT,dm,0,dU,nil,nil,dO,slower)end;

--------音频轨道标签。打标签之前的准备  dW是时间线 是dJ。dX是选择的文件位置
local function dV(dW,dm,dX,dO,slower)
print("xiaodai"..slower)
dX=d8(dX)
local dA=resolve:GetProjectManager()

local dB=dA:GetCurrentProject()
local dY=0;
local dU=dW:GetSetting("timelineFrameRate")

if type(dU)=="table"then print("fps is a table")dump(dU)
      dU=dU["timelineFrameRate"]end;
      print("fps:"..tostring(dU))
local dZ=dW:GetStartFrame()
      

    local function d_(e0)
      dY=dY+1;
      print("create markers for audio track item")print("start:"..e0:GetStart())
      print("end:"..e0:GetEnd())print("sequenceStart:"..dZ)
      print("left offset:"..e0:GetLeftOffset())
      print("right offset:"..e0:GetRightOffset())
      -- local dK=e0:GetStart()-e0:GetLeftOffset()-dW:GetStartFrame()
      ----local dK=0-e0:GetLeftOffset()
      -------local dM=e0:GetStart()-dZ;
      local dN=e0:GetRightOffset();

----------调用打标签函数
          dI(e0,dm,0,dU,nil,dN,dO,slower)end;

     local e1=dW:GetTrackCount("audio")
print("timeline has "..e1 .." audio tracks")


-------------处理循环dW=dB:GetCurrentTimeline()。e0是时间线上的片段
      for e=1,e1,1 do print("process audio track "..e)
      local e2=dW:GetItemListInTrack("audio",e)
      for dE,e0 in ipairs(e2)do local dF=e0:GetMediaPoolItem()
            if dF then local dG=dF:GetClipProperty("File Path")
            if type(dG)=="table"then dG=dG["File Path"]end;
            if dX==d8(dG)
            then d_(e0)end 
      end end end;
         if dY==0 then cG("未创建节拍标签","请确保音乐位于时间线上!\n"..dX)end end;

-------打时间线标签  dl是音频文件 #获取表的长度
local function e3(dm,dl,type,dO)

local dA=resolve:GetProjectManager()
local dB=dA:GetCurrentProject()
local e4=dy(dl)
print("number of items found with audio file "..#e4)
if#e4==0 then print("import music, since it does not exist in the project, yet")
      e4=resolve:GetMediaStorage():AddItemListToMediaPool(dl)end;

-------if type=="timelineMarker1"or type=="timelineMarker2" then 

    
      local dW=dB:GetCurrentTimeline()

        if dW==nil then cG("请创建或打开时间线",'只能在活动的时间线上打标签，并且时间线需要包含文件 "'..d5(dl)..'"')error("no active timeline")end;

       print("xiaobao"..type)

       dV(dB:GetCurrentTimeline(),dm,dl,dO,type)
       -----------获取timeline
-----dT是获取的媒体池音频文件
-- else 
--       for dE,dT in ipairs(e4)do dS(dT,dm,dO,type)end 
--       end 
end;

local function bg(bh,bi)
      local bj=fusion:MapPath("Temp:/")local e=1;local bk=bj..bh..e;while bb(bk)do e=e+1;bk=bj..bh..e end;if bi then os.execute('mkdir "'..bk..'"')end;return bk end;
      local function bl(bm,bn,bo)local function bp(bq)if bo==nil then return false end;for br,bs in ipairs(bo)do if bs==bq then return true end end;return false end;local bj=fusion:MapPath("Temp:/")math.randomseed(os.time())local e=math.random(100000,999999)local bk=bj..bm..e..bn;while bb(bk)or bp(bk)do e=e+1;bk=bj..bm..e..bn end;return bk end;
      
      local function e5(e6,u,e7,e8,e9)print("create beat clicks for quantized beats")
            local ea=d2().."BeatEditForResolve_Files"..a_.."audiofiles"..a_.."click.wav"
            local eb=44100;
            local ec=60/e6;local ed=dx(eb*ec)local ee=dx(eb*(e7+10))local ef=580;local eg=ed-(dx(eb*u)-ef)while eg<0 do eg=eg+ed end;
            dh('-i '..bR(e8)..' -i '..bR(ea)..[[ -filter_complex "[1:a]apad=whole_len=]]..ed..[[, atrim=end_sample=]]..ed..[[, aloop=loop=-1]]..[[:size=]]..ee..[[, atrim=start_sample=]]..eg..[[[loopedClicks];[0:a][loopedClicks]amix=inputs=2:duration=first:weights='0 1':normalize=0,volume=3dB" -ac 1 ]]..bR(e9),{expectFile=e9})
            if not b9(e9)then print("ffmpeg seems to have failed, file does not exist:")print(e9)cG("Could not create beat click audio file:","please check the console for details")error("ffmpeg failed to create beat click audio file")end end;
            
            local function eh(dm,e8,e9,bC)print("create beat clicks for unquantized beats")print(dm)if not bC then bC={}end;
            local c3=b3()
            local ea=d2().."BeatEditForResolve_Files"..a_.."audiofiles"..a_.."click.wav"local ei=''local ej=''local ek=''local el=1;local em=20;local en=nil;
            local eo=true;local ep="13152us"local eq={}
      local er=""
      
      local function es(e)print("run ffmpeg job "..e)
            local et=bl("BeatEditBeatClicks",".wav",eq)
            table.insert(eq,et)local eu;
            if eo then eu='-i '..bR(ea)..' -filter_complex "asplit='..el..' '..ei..'; '..ek..' '..ej..' amix=inputs='..el..':normalize=0[FINAL]" -map "[FINAL]" '..bR(et)else eu='-i '..bR(ea)..' -i '..bR(en)..' -filter_complex "[0:a]asplit='..el..' '..ei..'; '..ek..' '..ej..'[1:a] amix=inputs='..el+1 ..':normalize=0[FINAL]" -map "[FINAL]" '..bR(et)end;
            ---------c3是win

            if not c3 or bC.avoidBatFiles then dh(eu,{expectFile=et,avoidBatFiles=true})
                  if not b9(et)then print("ffmpeg seems to have failed, file does not exist:")
                  print(et)cG("Could not create beat click audio file:","please check the console for details")
                  error("ffmpeg failed to create beat click audio file")end;
      if not eo then os.remove(en)end else er=er.."\n"..de(eu)if not eo then er=er..'\ndel "'..en..'"'end end;en=et end;for e,dw in ipairs(dm)do local ev=dx(dw*1000)if ev<0 then ev=0 end;ei=ei..'[beat'..e..']'ej=ej..'[beat'..e..'o]'ek=ek..' [beat'..e..']adelay='..ev..':all=1[beat'..e..'o];'if el==em then es(e)eo=false;el=0;ei=''ej=''ek=''end;el=el+1 end;print("processed "..el.."beats")el=el-1;if el>0 then es(#dm)end;
      
      if c3 and not bC.avoidBatFiles then d9(er,true)
            if not b9(en)then print("bat file execution seems to have failed --> try alternative method")bC.avoidBatFiles=true;return eh(dm,e8,e9,bC)end end;dh('-i '..bR(e8)..' -i '..bR(en)..[[ -filter_complex "[1:a] atrim=start=]]..ep..[[[trimmedClicks];[0:a][trimmedClicks]amix=inputs=2:duration=first:weights='0 1':normalize=0,volume=3dB" -ac 1 ]]..bR(e9),{expectFile=e9})if not b9(e9)then print("ffmpeg seems to have failed, file does not exist:")print(e9)cG("Could not create beat click audio file:","please check the console for details")error("ffmpeg failed to create beat click audio file")end;os.remove(en)end;local function ew(ex)local ey={}for ez,eA in pairs(ex)do if type(eA)=='number'then table.insert(ey,eA)end end;table.sort(ey)if math.fmod(#ey,2)==0 then return(ey[#ey/2]+ey[#ey/2+1])/2 else return ey[math.ceil(#ey/2)]end end;local function eB(ex)local eC=0;local eD=0;for ez,eA in pairs(ex)do if type(eA)=='number'then eC=eC+eA;eD=eD+1 end end;return eC/eD end;local function eE(eF)local eG={}local bE={}for dE,eA in ipairs(eF)do if not eG[eA]then bE[#bE+1]=eA;eG[eA]=true end end;return bE end;local function eH(eI)return{h=math.floor(eI/3600),m=math.floor(eI%3600/60),s=math.floor(eI%60),ms=math.floor(eI%1*1000)}end;local function eJ(eI)local function eK(X,S)X=""..X;while string.len(X)<S do X="0"..X end;return X end;local ex=eH(eI)return""..eK(ex.h,2)..":"..eK(ex.m,2)..":"..eK(ex.s,2)..","..eK(ex.ms,3)end;local function eL(dm,dX)dX=d8(dX)local eM={}local dA=resolve:GetProjectManager()local dB=dA:GetCurrentProject()local dW=dB:GetCurrentTimeline()if dW==nil then cG("请打开时间线",'只有存在活动的时间线才能创建字幕文件，并且时间线需要包含文件 "'..d5(dX)..'"')error("no active timeline")end;local dU=dW:GetSetting("timelineFrameRate")local eN=1/dU;local function eO(dK,dL,dM,dN)local eP=dK/dL;local eQ=dN/dL;local eR=dM/dL;for dE,dw in ipairs(dm)do local cY=dw+eP;if cY>eQ then return end;if eR<=cY then table.insert(eM,cY)end end end;local dY=0;local dZ=dW:GetStartFrame()local function d_(e0)dY=dY+1;local dK=e0:GetStart()-e0:GetLeftOffset()-dW:GetStartFrame()local dM=e0:GetStart()-dZ;local dN=e0:GetEnd()-dZ;eO(dK,dU,dM,dN)end;local e1=dW:GetTrackCount("audio")for e=1,e1,1 do print("process audio track "..e)local e2=dW:GetItemListInTrack("audio",e)for dE,e0 in ipairs(e2)do local dF=e0:GetMediaPoolItem()if dF then local dG=dF:GetClipProperty("File Path")if type(dG)=="table"then dG=dG["File Path"]end;if dX==d8(dG)then d_(e0)end end end end;if dY==0 then cG("未创建字幕","请确保音乐位于时间线上!\n"..dX)end;

local function eS()local br=1;local bE=""if eM[1]>0 then bE=bE.."\n\n"..br.."\n"..eJ(0).." --> "..eJ(eM[1]).."\n".."before first beat".."\n"br=br+1 end;local eT=nil;print("found total number of beats for title:"..#eM)local eU=1;for e,eV in ipairs(eM)do print("process title time "..e)if eT~=nil then local eW=eJ(eT)local eX=eJ(eV)local bB="beat "..eU.."\n"..(eU-1)%4+1 .."/4\n"..(eU-1)%8+1 .."/8\n"..(eU-1)%16+1 .."/16"bE=bE.."\n\n"..br.."\n"..eW.." --> "..eX.."\n"..bB.."\n"br=br+1;eU=eU+1 end;print("process title time "..e.."DONE")eT=eV end;return bE end;table.sort(eM)eM=eE(eM)return eS()end;local function eY(dm,dX,eZ)local e_=eL(dm,dX)print("SRT:")print(e_)print("")print("write to file "..eZ)local bJ,b7=io.open(eZ,"w")if bJ==nil then print("could not open file for writing"..eZ)print(b7)error(b7)end;bJ:write(e_)bJ:close()return eZ end;local function f0(dm,dX,eZ)eY(dm,dX,eZ)cG("只差一步! :-)",'请将文件 "'..d5(eZ)..'" 拖入字幕轨道上，以查看拍子。记得让字幕文件位于时间线开端，这样才能对齐拍子.')bmd.openfileexternal('Open',d7(eZ))end;

local function f1(bA,dz,f2,f3)
      local itm;
      local f4=d7(dz)local function f5()return f4 ..a_..itm.Filename.Text end;

local t,bD=520,120;if fusion==nil then print("[Fusion Error] Please open up the Fusion GUI before running this tool.\n")return dz 
else if not ui then ui=app.UIManager;disp=bmd.UIDispatcher(ui)end;local f6=nil;
local bF=disp:AddWindow({ID="saveDlg",Target="saveDlg",WindowTitle=tostring(bA),
ui:VGroup{Spacing=4,ui:HGroup{Weight=0.0,ui:Label{ID="FilenameLabel",Text=tostring("文件名"),Weight=0},
ui:LineEdit{ID='Filename',Text=d5(dz)}},ui:HGroup{Weight=1,
ui:Label{ID="FolderLabel",Text=tostring("保存位置 "..f4),Weight=0.9,WordWrap=true},ui:Button{ID="FolderButton",Text="浏览",Weight=0.1}},ui:HGroup{Weight=1,

ui:Button{ID="SaveButton",Text="保存文件"}}}})bF:Resize({t,bD})
itm=bF:GetItems()
function bF.On.FolderButton.Clicked(bG)print('Choose Folder Button Clicked')
      local bE=tostring(fu:RequestDir(f4))if bE then f4=bE;print('[Folder] ',f4)itm.FolderLabel.Text="保存位置"..f4 end end;
function bF.On.saveDlg.Close(bG)disp:ExitLoop()end;function bF.On.SaveButton.Clicked(bG)f6=true;disp:ExitLoop()end;bF:Show()disp:RunLoop()bF:Hide()local function f7(am,f2)return d6(am).."."..f2 end;if f6 then local f8=f7(f5(),f2)if f3 then if b9(f8)then if bz("替换现有文件？","文件已存在，将会被替换:\n"..f8)
      then os.remove(f8)else return nil end end end;return f8 end;return nil end end;local function f9(e6,u,fa)local fb={}local ec=60/e6;for ex=u,fa,ec do table.insert(fb,ex)end;return fb end;

local function fc(dm)
      local function fd(u,ec)
            local fe=0;local ff=ec/2;local fg=0.1;for dE,dw in ipairs(dm)do local fh=math.abs((dw-u)%ec)
                  if fh>ff then fh=ec-fh end;
                  fh=math.min(fh,fg)fe=fe+fh 
            end;
                  return fe 
            end;
            local function fi(e6)local ec=60/e6;local fj=fd(0,ec)local fk=0;local fl=1/100;local fm=fl;while fm<ec do local fn=fd(fm,ec)if fn<fj then fj=fn;fk=fm end;
            fm=fm+fl end;return{bestError=fj,bestOffset=fk}
      end;
local function fo()
      local fp=5;local fq={}local fr=nil;for dE,dw in ipairs(dm)do if fr~=nil then local a4=dw-fr;table.insert(fq,60/a4)end;fr=dw end;local fs=ew(fq)local ft={}for dE,fv in ipairs(fq)do if math.abs(fv-fs)<fp then table.insert(ft,fv)end end;
return dx(eB(ft))end;
local e6=fo()
local fw=nil;
for fv=e6-5,e6+5,1 do local fx=fi(fv)
      if fw==nil or fw["bestError"]>fx["bestError"]then fw=fx;fw["bpm"]=fv end end;
return{bpm=fw.bpm,offset=fw.bestOffset}
end;
local function fy(dR,dU)
      local fz=dR*1/dU;
      local fA=string.format("%02.f",math.floor(fz/3600))local fB=string.format("%02.f",math.floor(fz/60-fA*60))local fC=string.format("%02.f",math.floor(fz-fA*3600-fB*60))local fD=string.format("%02.f",dR%dU)return fA..":"..fB..":"..fC..":"..fD end;
-------

-------dW当前时间线dX是音频文件
local function fE(dW,dm,dX)
      local fF=nil;dX=d8(dX)
      local dA=resolve:GetProjectManager()
      local dB=dA:GetCurrentProject()
      local dY=0;local dU=dW:GetSetting("timelineFrameRate")
      if type(dU)=="table"then print("fps is a table")dump(dU)dU=dU["timelineFrameRate"]end;
      print("fps:"..tostring(dU))
      local dZ=dW:GetStartFrame()
      local function fG(dK,dM,dN)
            for e,dw in ipairs(dm)do local dR=dx(dw*dU)+dK;
                  if dR>dN then return nil end;
                  if dM<=dR then print("found beat at frame "..dR)
                  return dR end end;
            return nil end;
      local function d_(e0)
                  dY=dY+1;
                  print("find first beat in audio track item")
                  print("start:"..e0:GetStart())
                  print("end:"..e0:GetEnd())
                  print("sequenceStart:"..dZ)print("left offset:"..e0:GetLeftOffset())
                  
                  print("right offset:"..e0:GetRightOffset())
                  local dK=e0:GetStart()-e0:GetLeftOffset()-dW:GetStartFrame()
                  local dM=e0:GetStart()-dZ;
                  local dN=e0:GetEnd()-dZ;
                  return fG(dK,dM,dN)end;
            local function fH(e)print("process audio track "..e)
                  local e2=dW:GetItemListInTrack("audio",e)for dE,e0 in ipairs(e2)do local dF=e0:GetMediaPoolItem()
                  if dF then local dG=dF:GetClipProperty("File Path")
                        if type(dG)=="table"then dG=dG["File Path"]end;
            if dX==d8(dG)then local fI=d_(e0)if fI~=nil then if fF==nil or fF>fI then fF=fI;return end end end end end end;
            local e1=dW:GetTrackCount("audio")print("timeline has "..e1 .." audio tracks")for e=1,e1,1 do fH(e)end;if fF~=nil then fF=fF+dZ end;return fF end;

----dl音频文件名
local function fJ(dm,dl)
      local dA=resolve:GetProjectManager()
      if dA==nil then return nil end;
      local dB=dA:GetCurrentProject()
      if dB==nil then return nil end;
      local dW=dB:GetCurrentTimeline()
      if dW==nil then return nil end;
      local dR=fE(dW,dm,dl)
      if dR==nil then return nil end;
      local dU=dW:GetSetting("timelineFrameRate")if type(dU)=="table"then print("fps is a table")dump(dU)
            dU=dU["timelineFrameRate"]end;
            print("fps:"..tostring(dU))
            return fy(dR,dU)end;
...
local t,bD=600,200;local fL='请选择音频文件...'local dm=nil;local fM=nil;local fN=nil;

local bF=disp:AddWindow({ID='BeatEditWindow',WindowTitle='自动踩点 1.2.1',Spacing=10,
ui:VGroup{ID='root',Weight=0,
ui:HGroup{ui:Label{ID='StatusMessage',Text='尚未获取数据',Weight=0.125},

ui:Button{Weight=0,ID='SettingsButton',Flat=false,Text='@Sunday映像',Margin=0,MinimumSize={128,32}

}
},

ui:HGroup{ui:Label{ID='FileLabel',Text='音乐文件:',Weight=0.125},
ui:Label{ID='FileTxt',Text=fL,Weight=0.75,WordWrap=true},
ui:Button{ID='FileButton',Text='选择文件',Weight=0.125}},
ui:HGroup{ui:ComboBox{ID='CreateType',Weight=0.875},ui:Button{ID='CreateButton',Text='创建标签',Weight=0.125}},ui:HGroup{ui:CheckBox{ID='QuantizeCheckbox',Text='假定拍子无变化',Weight=1,Checked=true,ToolTip="只有在歌曲拍子会变化时才需要去除勾选（大多数情况下歌曲拍子是固定的）"},ui:CheckBox{ID='SlowCheckbox',Text='标签比拍子慢一帧',Weight=1,Checked=true,ToolTip="用声音唤起画面，获得更好的视听效果"}}
}})

bF:Resize({t,bD})
local itm=bF:GetItems()
itm.CreateType:AddItem('在音频上踩拍子Ⅰ和Ⅱ')
itm.CreateType:AddItem('在音频上踩拍子Ⅰ')
itm.CreateType:AddItem('在音频上踩拍子Ⅱ')
itm.CreateType:AddItem('创建音频拍子')

itm.CreateButton:SetEnabled(false)
local fO=""
local dO={setMessage=function(bB,fP)
      if not fP then fO=bB end;itm.StatusMessage.Text=bB end,clearTemporaryMessage=function()itm.StatusMessage.Text=fO end}

function bF.On.BeatEditWindow.Close(bG)print("close pressed")disp:ExitLoop()end;

function bF.On.SettingsButton.Clicked(bG)

cE(''..'https://space.bilibili.com/24892290')

end;

local function fQ(fR)
      itm.CreateButton:SetEnabled(fR)itm.FileButton:SetEnabled(fR)end;
      
      function bF.On.FileButton.Clicked(bG)
            fQ(false)print('Open File Button Clicked')
local dl=fu:RequestFile()
if dl==nil then print("user canceled")
      itm.CreateButton:SetEnabled(dm~=nil)
      itm.FileButton:SetEnabled(true)
      return end;
...
print('[File] ',dl)
itm.FileTxt.Text=dl;
dO.setMessage("拍子检测中...",false)
if not pcall(function()
      dm=dk(dl)end)
      then dO.setMessage("拍子检测失败",false)
      itm.FileButton:SetEnabled(true)return 
end;
      dO.setMessage("检测拍子中.....",false)
      fN=fc(dm)
      dO.setMessage("检测拍子中......",false)

      ----------fM是拍子信息
fM=f9(fN["bpm"],fN["offset"],dm[#dm])
print("-------------------------------")
print("检测完成")
print("BPM:"..fN["bpm"])
print("offset:"..fN["offset"])print("-------------------------------")

--------fS音频轨道开始时间
local fS=fJ(fM,dl)
local fT;
if fS==nil then fT="音乐不在时间线上"
else fT="音乐开始于"..fS end;
dO.setMessage("已完成（节拍速度为 "..fN["bpm"].." bpm; "..fT.."）")
fQ(true)end;
function bF.On.CreateButton.Clicked(bG)print('Marker Button Clicked')if dm==nil then cG("请选择音频文件","要创建拍子标签，你需要选择音频文件")return end;fQ(false)

local function fU()
local dl=itm.FileTxt.Text;
local fV;

if itm.CreateType.CurrentIndex==0 and itm.SlowCheckbox.Checked
            then fV="timelineMarker1"
            elseif itm.CreateType.CurrentIndex==0 and not itm.SlowCheckbox.Checked
            then fV="timelineMarker2"
            elseif itm.CreateType.CurrentIndex==1 and itm.SlowCheckbox.Checked
            then fV="clipMarker1"
            elseif itm.CreateType.CurrentIndex==1 and not itm.SlowCheckbox.Checked
            then fV="clipMarker2"
            elseif itm.CreateType.CurrentIndex==2 and itm.SlowCheckbox.Checked
            then fV="clipMarker3"
            elseif itm.CreateType.CurrentIndex==2 and not itm.SlowCheckbox.Checked
            then fV="clipMarker4"
            elseif itm.CreateType.CurrentIndex==3 then fV="beatClickAudio"
            else error("unsupported create option")
            end;
...
if itm.QuantizeCheckbox.Checked 

then fW=fM else fW=dm end;

if fV=="timelineMarker1"or fV=="clipMarker1"or fV=="timelineMarker2"or fV=="clipMarker2"or fV=="clipMarker3"or fV=="clipMarker4"
then e3(fW,dl,fV,dO)


elseif fV=="beatClickAudio"then 
local fX=d6(dl).."_beat_clicks.wav"
local fY=f1("将拍子文件保存",fX,"wav",true)
if fY~=nil then 
if itm.QuantizeCheckbox.Checked then e5(fN["bpm"],
fN["offset"],dm[#dm],dl,tostring(fY))
else eh(fW,dl,tostring(fY))end;
cG("只差一步! :-)",'请将文件 "'..d5(fY)..'" 拖入你的音频轨道，以监听音频拍子， 记得将拍子音频文件与你的音乐文件对齐')
bmd.openfileexternal('Open',d7(fY))end 

-------保存字幕选项
elseif fV=="subtitle"then local fZ=d6(dl)..".srt"
      local eZ=f1("将拍子保存为字幕文件",fZ,"srt",true)
           if eZ~=nil then f0(fW,dl,eZ)end 

      else error("not implemented create option")end 
end;

pcall(fU)fQ(true)
end;
bF:Show()disp:RunLoop()bF:Hide()
